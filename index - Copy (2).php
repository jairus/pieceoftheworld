	...
	function onDraggableRectangleChanged(bounds) {
		...
		if (bounds||(lastEvent.getTime() + 250 <= new Date().getTime())) {
			...
			if (typeFirstBlock == 1) {
				...
				jQuery('#buy-button').attr("disabled", false);
				// Empty Plot
				var maxX = 0;
				var maxY = 0;
			...
			*/
			...
			jQuery('#info-latitude').html(LatLngCenter.lat().toFixed(5));
			jQuery('#info-longitude').html(LatLngCenter.lng().toFixed(5));
			...
			if(typeFirstBlock == 1){
				...
				if(numblocks > 0){
					...
					jQuery('#info-detail')html(jQuery('#info-detail')html()+'<br /><br />Price: $'+price);
				}
			}
	...
	function updatePopupWindowTabInfo(inLatLng) {
		...
		if (returnText != '[[]]') {
			...
			if(markerJSON[0].thumb_url){
				...
				jQuery('#info-img').attr("src","images/thumbs/land_id_"+markerJSON[0].id+"_thumb?_="+((new Date()).getTime()));
			...
			if (markerJSON[0].email == masterUser) {
				if(markerJSON[0].land_special_id){
					price = 499;
					jQuery('#info-detail').html(document.getElementById('info-detail').innerHTML + '<br /><br />Price: $'+price);
				}
				else if(numblocks > 0){
					price = (numblocks*9.90);
					price = price.toFixed(2);
