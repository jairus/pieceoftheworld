...
<?php } ?>
	...
	function onIdle(manualCall) {
		...
		onIdleId = (new Date()).getTime();
		consoleX("onIdle"+" "+onIdleId);
		// Remove rectangles if exist
		if(map.getZoom() <= 16){
			while (rectangles.length > 0) {
				//rectangles.pop().setOptions({fillOpacity: 0})
				rectangles.pop().setMap(null);
			}
			ixjx = [];
		}
		if (map == null) { return; }
		
		consoleX(map.getZoom());
	...
	function drawRect(lt, rb, color, opacity) {
		...
		//if(recltrb.indexOf(lt+""+"-"+""+rb)==-1){
			window.recltrb.push(lt+""+"-"+""+rb);
	...
	function dBlocksLoop(jinit, ix, jx, t1, t2, onIdleIdlocal,
	returnText, markersJSON, config_showownedland, config_showimportantplaces, config_showownland
	){
		 
		if(onIdleId!=onIdleIdlocal){
			return 0;
		}
		/**** codes here ****/
		if(ixjx.indexOf(ix+"-"+jx)==-1){
			cnt1 = ix;
			cnt2 = jx;
			color = "";
			opacity = 0;
			...
			//consoleX(jinit+" - "+ix+" "+jx+" "+t1+" "+t2+" - "+onIdleIdlocal)
		}
		/*******************/
		ixjx.push(ix+"-"+jx)
		jx++;
		...
		if(ix<t1){
			setTimeout(function(){ dBlocksLoop(jinit, ix, jx, t1, t2, onIdleIdlocal, 
			returnText, markersJSON, config_showownedland, config_showimportantplaces, config_showownland
			); }, .1);
		}
	}
	...
	function drawBlocks(map) {
		...
		h = hEnd - hStart;
		hminus = Math.round(h/3);
		
		v = vEnd - vStart;
		vminus = Math.round(v/3);
		
		hStart1 = hStart;
		hEnd1 = hEnd - hminus - hminus;
		hStart2 = hStart + hminus;
		hEnd2 = hEnd - hminus;
		hStart3 = hStart + hminus + hminus;
		hEnd3 = hEnd;
		
		vStart1 = vStart;
		vEnd1 = vEnd - vminus - vminus;
		vStart2 = vStart + vminus;
		vEnd2 = vEnd - vminus;
		vStart3 = vStart + vminus + vminus;
		vEnd3 = vEnd;

		if(1){
			dBlocksLoop(
				hStart, vStart, hStart, vEnd, hEnd, onIdleId,
				returnText, markersJSON, config_showownedland, config_showimportantplaces, config_showownland
			);
			
			/*
			dBlocksLoop(
				hStart1, vStart1, hStart1, vEnd1, hEnd1, onIdleId,
				returnText, markersJSON, config_showownedland, config_showimportantplaces, config_showownland
			);
			
			dBlocksLoop(
				hStart2, vStart2, hStart2, vEnd2, hEnd2, onIdleId,
				returnText, markersJSON, config_showownedland, config_showimportantplaces, config_showownland
			);
			
			dBlocksLoop(
				hStart3, vStart3, hStart3, vEnd3, hEnd3, onIdleId,
				returnText, markersJSON, config_showownedland, config_showimportantplaces, config_showownland
			);
			*/
		}
