...
?>
	...
	function onIdle(manualCall) {
		...
		if (map.getZoom() >= ZOOM_LEVEL_CITY) {
			...
			//consoleX(getCookie("config_showgrid"));
			if (getCookie("config_showgrid") != "false"||1) {
				drawBlocks(map);
			}
		}
		else {
			//consoleX("else city level");
			if (manualCall) {
	...
	function dBlocksLoop(jinit, i, j, t1, t2, onIdleIdlocal){
		...
		color = "";
		opacity = 0;
		var result = getBlockLTRB(new google.maps.Point(j, i));
		/*
		if (returnText != '[[]]') {
		...
		*/
		...
		if(i<t1){
			setTimeout(function(){ dBlocksLoop(jinit, i, j, t1, t2, onIdleIdlocal); }, .1);
		}
	}
	
	//dBlocksLoop(0, 0, 10, 10);
	...
	function drawBlocks(map) {
		...
		var email;
		i = 0;
		consoleX(" vStart="+vStart+" vEnd="+vEnd+" hStart="+hStart+" hEnd="+hEnd);
		
		dBlocksLoop(hStart, vStart, hStart, vEnd, hEnd, onIdleId);
